...
<body>
    ...
    $userfetch = $result->fetch_assoc(); // Fetch the row from the result, not $userrow
    if ($userfetch) {
        ...
        $userrow = $database->query("SELECT * FROM patient WHERE pid = '$userid'");
        if ($userrow) {
            ...
            // Handle case where the query didn't return any result
            echo "No user found";
        }
    } else {
